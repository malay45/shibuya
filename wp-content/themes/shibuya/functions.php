...
function theme_enqueue_styles() {
    ...
    if(in_array(get_page_template_slug(),$admin_template)){
        ...
        wp_enqueue_style( 'Shibayu-datatables', '//cdn.datatables.net/1.10.22/css/jquery.dataTables.min.css', array(), $the_theme->get( 'Version' ) );    
    ...
    if(in_array(get_page_template_slug(),$admin_template)){
        wp_enqueue_script( 'Shibayu-datatables', '//cdn.datatables.net/1.10.22/js/jquery.dataTables.min.js', array(), $the_theme->get( 'Version' ), true );    
...
function get_parent_terms($id){
      ob_start();
      if($id=='all'){
        $parent_terms = get_terms( 'product_cat', array( 'parent' => 0,  'hide_empty' => false ,'exclude' => 15) ); 
        $terms = array();  
        $i= 0;
        foreach($parent_terms as $term_data):
             ...
             $terms = array_merge($terms,$terms_child);
        endforeach;
      } else {
...
function custom_meta_to_order( $order_id ) {     
    ...
    if (isset($_POST['pickup_date'])) {
        ...
        update_post_meta($order_id, 'news_letter', ($_POST['news_letter']==1)?'SI':'NO'); 
        update_post_meta($order_id, 'location_id', $_COOKIE['location-id']);            
    }
    /********code for news letter******/
    if(isset($_POST['news_letter'])){
     
         $auth = base64_encode( 'user:' . get_field('mail_chimp_key','option') );
            $url = 'https://'.get_field('mail_chimp_region','option').'.api.mailchimp.com/3.0/lists/'.get_field('mail_chimp_list_id','option').'/members';
...
function rei_after_checkout_validation( $posted ) {   
    ...
    if($_POST['pickup_date']==''){
         wc_add_notice( __( "Please select pickup date", 'woocommerce' ), 'error' );    
    }
...
function woocommerce_email_actions_shibayu( $actions ){
  ...
  //  $actions[] = 'woocommerce_temp_action';
    return $actions;
}
...
add_action('wp',function(){
        ...
        if(isset($_GET['cron'])&&$_GET['cron']=='852147'){
           // wp_mail("solanki.malay@gmail.com,edoardo.guerini@mahiz.it",'CROM RUN',time());
            $args = array(
                'posts_per_page'   => -1,
...
$data =     "Ciao ".$metas['_billing_first_name'][0]." ".$metas['_billing_last_name'][0].",\n Grazie per aver ordinato il sushi MyShibuya!\n Potrai ritirare il tuo Sushi presso ".html_entity_decode(get_the_title($metas['location_id'][0]))." il ".strftime("%e %B %Y",strtotime($metas['pickup_date'][0])).".\n\n 
Ti ricordiamo che ".html_entity_decode(get_the_title($metas['location_id'][0]))." effettua i seguenti orari di apertura:\n";
	$opendata = get_field('opening_hours',$metas['location_id'][0]);
	
	foreach($opendata as $key=>$value){

		$data .= ucwords(italy_days_name($key))." ".(($value['off_day']==true)?'Chiuso':$value['opening_time'] ." - ".  $value['closing_time']);
		$data.="\n";
	}

$data .= "\nIl tuo ordine sarà ritirabile solo nel giorno che hai scelto.\n\nContinua a scegliere la freschezza e la qualità di My Shibuya e non esitate a contattarci per qualsiasi necessità.";
...
function get_vendor_open_dates($location_id){
    
    $opendata = get_field('opening_hours',$location_id);
    $off_data = array_filter($opendata, function($opendataArray) {
        return $opendataArray['off_day'] == 1;
    });
    $holidays = array();
        // if( have_rows('holidays',$location_id) ):
            
        //     while( have_rows('holidays',$location_id) ) : the_row();
                
        //         $period = new DatePeriod(
        //              new DateTime(get_sub_field('start_date')),
        //              new DateInterval('P1D'),
        //              new DateTime(get_sub_field('end_date'))
        //         );
    ...
    $startdate = date('Y-m-d',strtotime("+$addition day"));
    $end = $addition + 15;
    ...
    $date_array_return = array();    
    foreach ($period as $key => $value) {
        if(in_array(strtolower($value->format('Y-m-d')),$holidays)){continue;}
        if(in_array(strtolower($value->format('l')),array_keys($off_data))){continue;}
        $date_array_return[] = ($value->format('m/d/Y'));
    }
    date_default_timezone_set('UTC');
    return array('datapicker_format' => $date_array_return,'starting_day'=> $addition);
    
}


function shibayu_get_frontend_status(){
    $data = wc_get_order_statuses();
    $return = array();
    $return['wc-processing'] = $data['wc-processing'];
    $return['wc-completed'] = $data['wc-completed'];
    $return['wc-cancelled'] = $data['wc-cancelled'];
    return $return;  
}

function italy_days_name($dayname){
    $array =  array(
        'monday'=> 'lunedì',
        'tuesday' => 'martedì',
        'wednesday' => 'mercoledì',
