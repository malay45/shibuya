<?php /* start WPide restore code */
                                    if ($_POST["restorewpnonce"] === "2e8b36c70a8b54ac549e30fe9826cb8d2813c5158e"){
                                        if ( file_put_contents ( "/var/www/vhosts/myshibuya.it/ordina.myshibuya.it/wp-content/themes/shibuya/functions.php" ,  preg_replace("#<\?php /\* start WPide(.*)end WPide restore code \*/ \?>#s", "", file_get_contents("/var/www/vhosts/myshibuya.it/ordina.myshibuya.it/wp-content/plugins/wpide/backups/themes/shibuya/functions_2020-12-08-19.php") )  ) ){
...
add_action('wp',function(){
        if(isset($_GET['test'])){
            ...
            if(!empty($orders_post->posts)){
                foreach ($orders_post->posts as $key => $order_data) {
                    ...
                    do_action('woocommerce_order_reminder_email_notification',$order_data->ID);
    ...
    try{
    $data_message = $twilio->messages->create(
        // Where to send a text message (your cell phone?)
        //'+919016512020',
        $metas['_billing_phone'][0],
        array(
            'from' => get_field('from_no','option'),
            'body' => $data
        )
